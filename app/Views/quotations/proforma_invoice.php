...
<?php if (!empty($quotation['bank_account_number'])): ?><p style="border:1px solid #bbb;padding:8px;"><strong>Informasi Pembayaran</strong><br>Bank: <?= esc($quotation['bank_name'] ?? '-') ?><br>Nama rekening: <?= esc($quotation['bank_account_name'] ?? '-') ?><br>No. rekening: <?= esc($quotation['bank_account_number'] ?? '-') ?><?= !empty($quotation['bank_branch']) ? '<br>Cabang: '.esc($quotation['bank_branch']) : '' ?></p><?php endif; ?>
