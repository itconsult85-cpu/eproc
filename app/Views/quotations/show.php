...
<?php foreach ($quotation['negotiations'] as $negotiation): ?><tr><td><?= esc($negotiation['round_no']) ?></td><td><span class="badge text-bg-<?= $negotiation['status'] === 'accepted' ? 'success' : ($negotiation['status'] === 'rejected' ? 'danger' : 'warning') ?>"><?= esc(ucfirst($negotiation['status'])) ?></span></td><td><?= esc($negotiation['proposed_by']) ?></td><td><?= esc($negotiation['customer_message'] ?: '-') ?></td><td>Rp <?= number_format((float)$negotiation['grand_total'], 0, ',', '.') ?></td><td><?php if ($negotiation['status'] === 'pending'): ?><form method="post" action="<?= esc(site_url('quotations/' . $quotation['id'] . '/negotiations/' . $negotiation['id'] . '/accepted')) ?>" class="d-inline" data-confirm data-confirm-title="Terima sebagai kondisi final?" data-confirm-message="Kondisi negosiasi ini akan diterapkan sebagai nilai final quotation dan statusnya menjadi disetujui." data-confirm-label="Ya, terima & final" data-confirm-variant="success"><?= csrf_field() ?><button class="btn btn-sm btn-success">Terima & Final</button></form> <form method="post" action="<?= esc(site_url('quotations/' . $quotation['id'] . '/negotiations/' . $negotiation['id'] . '/rejected')) ?>" class="d-inline" data-confirm data-confirm-title="Tolak putaran negosiasi?" data-confirm-message="Putaran negosiasi ini akan ditandai sebagai ditolak dan quotation dikembalikan ke status terkirim." data-confirm-label="Ya, tolak" data-confirm-variant="danger"><?= csrf_field() ?><button class="btn btn-sm btn-outline-danger">Tolak</button></form><?php else: ?><?= esc($negotiation['responded_at'] ?? '-') ?><?php endif; ?></td></tr><?php endforeach; ?>
</tbody></table></div></div>
<?php endif; ?>
<?= $this->endSection() ?>
