...
<?php if ($quotation['status'] === 'approved' && can('quotations.status')): ?><div class="card border-success mb-3"><div class="card-body"><div class="d-flex flex-wrap justify-content-between align-items-center gap-2"><div><h2 class="h6 mb-1"><i class="bi bi-bank me-1"></i>Rekening Pembayaran Client</h2><p class="small text-body-secondary mb-0">Pilih rekening yang akan ditampilkan pada PDF Proforma Invoice final.</p></div><form method="post" action="<?= esc(site_url('quotations/' . public_id($quotation['id']) . '/bank-account')) ?>" class="d-flex align-items-center gap-2"><?= csrf_field() ?><select name="bank_account_id" class="form-select" data-searchable-select data-placeholder="Pilih rekening" required><option value=""></option><?php foreach (($bankAccounts ?? []) as $bank): ?><option value="<?= $bank['id'] ?>" <?= (int)($quotation['bank_account_id'] ?? 0) === (int)$bank['id'] ? 'selected' : '' ?>><?= esc($bank['bank_name'].' · '.$bank['account_number'].' · '.$bank['account_name']) ?></option><?php endforeach; ?></select><button class="btn btn-success text-nowrap"><i class="bi bi-check2 me-1"></i>Simpan Rekening</button></form></div><?php if (!empty($quotation['bank_account_id'])): ?><div class="small text-success mt-2"><i class="bi bi-check-circle me-1"></i>Rekening terpilih akan tampil pada PDF Proforma Invoice.</div><?php else: ?><div class="small text-warning mt-2"><i class="bi bi-exclamation-circle me-1"></i>Belum ada rekening yang dipilih.</div><?php endif; ?></div></div><?php endif; ?>
