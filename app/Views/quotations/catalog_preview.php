...
<div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
    <div>
        ...
        <p class="text-body-secondary mb-0">Quotation <?= esc($quotation['quotation_no']) ?> · <?= count($items) ?>
            produk terpilih
        </p>
    </div>
...
<div class="card">
    <div class="card-body">
        <div class="border-bottom pb-3 mb-3">
            <?php if ($logoData): ?>
            <img src="<?= $logoData ?>" alt="Logo" style="max-width:180px;max-height:60px;object-fit:contain"
                class="mb-2">
            <br>
            <?php endif; ?>
            ...
            <div class="text-body-secondary"><?= esc($settings['company_name'] ?? '') ?> · Customer:
                <?= esc($quotation['customer_name'] ?: $quotation['company_name']) ?>
            </div>
        </div>
        <?php if (!$items): ?>
                ...
                <article class="border rounded p-3 h-100">
                    <div class="row g-3">
                        <div class="col-sm-3 col-md-2 text-center">
                            ...
                            <img src="<?= $entry['imageData'] ?>" alt="Produk" class="img-fluid rounded"
                        ...
                        <div class="col-sm-9 col-md-10">
                            <h2 class="h5 mb-1"><?= $index + 1 ?>. <?= esc($product['name'] ?? $item['product_name']) ?>
                            </h2>
                            ...
                            <p class="mb-2"><?= nl2br(esc($product['description'] ?? $item['description'])) ?></p>
                            <?php endif; ?>
                            <?php if ($specs): ?>
                            ...
                            <ul class="mb-2">
                                ...
                                <li><strong><?= esc($spec['label']) ?>:</strong>
                                    <?= nl2br(esc($spec['value'])) ?>
                                </li>
                                <?php endforeach; ?>
                            </ul>
                            <?php endif; ?>
                            <?php if (!empty($product['applications'])): ?>
                            <p class="mb-1"><strong>Aplikasi:</strong>
                                <?= nl2br(esc($product['applications'])) ?>
                            </p>
                            <?php endif; ?>
                            <?php if (!empty($product['standards'])): ?>
                            <p class="mb-1"><strong>Standar:</strong>
                                <?= nl2br(esc($product['standards'])) ?>
                            </p>
                            <?php endif; ?>
                            <?php if (!empty($product['datasheet'])): ?>
                            <p class="mb-1"><strong>Datasheet:</strong>
                                <?= nl2br(esc($product['datasheet'])) ?>
                            </p>
                            <?php endif; ?>
